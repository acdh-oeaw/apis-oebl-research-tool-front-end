...
export const getHeaders = async (
	...
	if (isStringWithValue(token)) {
		/** FIXME: This was changed to work with django rest framework. */
...
export const request = <T>(
	...
	return new CancelablePromise(async (resolve, reject, onCancel) => {
		// FIXME: this was added - is that actually needed?
		if (["post", "put", "patch", "delete"].includes(String(options.method).toLowerCase())) {
		...
		try {
			...
			console.error(error);
			confirm.confirm("Serverfehler. Details in der Console.", {
				showCancel: false,
				icon: "mdi-error",
			});
