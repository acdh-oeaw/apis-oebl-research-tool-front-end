...
export default class EditorLoader extends Vue {
  ...
  async loadArticleData(): Promise<void> {
    ...
    const newestVersion: LemmaArticleVersion | undefined =
      articleStore.newestVersion;
  ...
  get sortedVersionViews(): Array<{
    ...
    const versionsWithDate: SavedArticleVersionWithDate[] =
      this.articleStore.versions.map((version) => {
        return {
          id: version.id,
    ...
    return sortedVersions.map((version, index) => {
      return {
        ...
        isShownInEditor:
          this.versionToEdit !== null && version.id === this.versionToEdit.id,
      };
    });
  }
  ...
  selectVersion(versionId: number): void {
    ...
    const versionToSelect = this.articleStore.versions.find(
