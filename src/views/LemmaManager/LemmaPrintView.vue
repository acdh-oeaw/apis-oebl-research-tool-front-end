        ...
        <div class="main-data">
          <div class="name">
            ...
            <div class="field professionGroup">
              ...
              <div class="fieldvalue">{{ lemma.professionGroup ? lemma.professionGroup.name : '' }}</div>
            </div>
          </div>
        </div>
      </section>
  ...
  beforeCreate() {
    db.lemmas
      ...
      .then((serializedLemmaRow) => {
        ...
        zoteroBy.load(this.lemma.zoteroKeysBy).then(z => this.zoteroCitationsBy = z.zoteroItems.map(convertZoteroItemToView));
        zoteroAbout.load(this.lemma.zoteroKeysAbout).then(z => this.zoteroCitationsAbout = z.zoteroItems.map(convertZoteroItemToView));
      })
      .catch((error) => {
