		...
		<v-app-bar app color="background darken-1" class="elevation-0 pt-3 pr-3">
			...
			<div>
				...
				<div class="caption mt-1 text-no-wrap">
					<span style="opacity: 0.7">
						{{ this.issueLemmas.length }} Ergebnisse. {{ this.filteredIssues.length }} angezeigt
					</span>
				</div>
			</div>
			<v-spacer />
			...
			<v-menu content-class="soft-shadow" offset-y left bottom :close-on-content-click="false">
				<template v-slot:activator="{ on, attrs }">
					<v-btn v-bind="attrs" v-on="on" tile class="rounded-lg" icon>
			...
			<v-btn @click="showSideBar = !showSideBar" v-if="!showSideBar" tile class="rounded-lg" icon>
				...
				<v-row dense>
					<v-col>
						<v-btn
							...
							block
						>
							<v-icon style="opacity: 0.7" left>mdi-bookshelf</v-icon>
							löschen
						</v-btn>
					</v-col>
					<v-col>
						<v-btn class="rounded-lg" elevation="0" block color="primary">
							<v-icon style="opacity: 0.7" left>mdi-pen</v-icon>
							Artikel anzeigen
						</v-btn>
					</v-col>
				</v-row>
