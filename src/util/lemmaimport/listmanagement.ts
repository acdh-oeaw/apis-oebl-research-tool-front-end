...
export class ListManager {
    ...
    get lemmaLists(): ChosenLemmaList[] {
        ...
        return listsInOtherListFormat.map(
            (otherFormat) => {
                return {
                    id: otherFormat.id,  
    ...
    public async getLemmaList(title: string): Promise<ChosenLemmaList> {
        const lemmaLists = this.lemmaLists.filter(list => list?.title !== undefined && list.title === title);
        if (lemmaLists.length === 1) {
            return lemmaLists[0];
        }
        
        if (lemmaLists.length > 1) {
            console.warn(`Found ${lemmaLists.length} lemma lists with title <${title}> – these should be unique, Dude! (I am pretty sure ;-)`);
